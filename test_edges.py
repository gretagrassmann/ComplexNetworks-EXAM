...
#  test_list, test_data = cPickle.load(open(test_data_file))
  ...
  for model_num in model_list:
      with tf.Session() as sess:
         ...
         saver.restore(sess, './saved_models_Edges/model_%d.ckpt'%(model_num))
         print(" Using model with edges, model_number = %d "%(model_num)," for testing %d proteins"%(len(test_data)))
         ...
         for prot_data in test_data:
           ...
           for a_batch in batches:
             ...
             all_losses += [res[0]]
             with open('Testing_loss.txt','a+') as file:
                 if model_num == 0:
                     f.write('Average loss, epoch number \n')
                 f.write(str(avg_loss/ii)+', '+ str(model_num)+"\n")
